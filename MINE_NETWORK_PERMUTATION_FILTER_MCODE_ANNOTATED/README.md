...
| **Null distribution** | 30 000 shuffled random pairs, histogram MI | 10 000 shuffled random pairs, MINE | **1 000 permutations** (user spec), supports both **global** and **per-pair** modes |
...
| **Permutation target** | 30 000 per study | 10 000 per study | 1 000 per study/pair (user spec; adjustable) |
...
| **Permutation count** | 30 000 | 1 000 default (matching 0.001 threshold resolution); configurable |
...
logCPM → Z-score → [optional Pearson pre-screen]
       ...
       → 1K permutation null (global or per-pair)
...
└── AnnotationConfig    gmt_paths, fdr_threshold, min_overlap, background_genes
```

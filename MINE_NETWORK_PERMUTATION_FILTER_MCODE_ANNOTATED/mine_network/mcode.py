...
def leiden_modules(
    ...
    gene_names : list[str]
        Gene names aligned to adjacency indices.
    edge_weights : np.ndarray, optional
        Weighted adjacency matrix for Leiden optimization.
    resolution : float
        Leiden resolution parameter.
    n_iterations : int
        Number of iterations (-1 for igraph default behavior).
    min_size : int
        Filter out communities smaller than this size.
    """
    ...
    g = ig.Graph(n=adj_sym.shape[0], edges=list(zip(rows.tolist(), cols.tolist())), directed=False)
    ...
    try:
        ...
        # Backward-compatible parameter name in some igraph versions
        part = g.community_leiden(
            objective_function="modularity",
            weights=weights,
...
def refine_large_modules_with_mcode(
    ...
    """
    Rerun MCODE inside oversized modules and replace them with submodules.

    Returns
    -------
    refined_modules : dict[int, list[str]]
    refined_membership : dict[str, int]
    parent_child_rows : list[dict]
        Mapping rows with parent/child IDs and sizes.
    """
    if size_threshold is None:
        membership = _membership_from_modules(modules)
        return modules, membership, []

    print(f"[INFO] Refining modules larger than {size_threshold} genes with MCODE...")

...
def save_sample_qc_figure(
    ...
    # Keep distance matrix in [0, 2] and symmetrically valid for linkage
    ...
    # Panel 1: hierarchical clustering
    ...
    # Panel 2: line plot of sample distributions
    ...
    # Panel 3: heatmap of sample correlations in dendrogram order
    ax3 = fig.add_subplot(gs[0, 2])
    order = dend["leaves"]
    corr_ord = corr.values[np.ix_(order, order)]

    if corr_threshold is not None:
        # Keep only correlations passing |r| threshold for visual emphasis
